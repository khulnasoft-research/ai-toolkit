...
- A [ KhulnaSoft AI Gateway ](https://vercel.com/ai-gateway) API key.

If you haven't obtained your KhulnaSoft AI Gateway API key, you can do so by [signing up](https://vercel.com/d?to=%2F%5Bteam%5D%2F%7E%2Fai&title=Go+to+AI+Gateway) on the Vercel website.
...
Install `ai` and `@ai-toolkit/svelte`, the AI package and AI TOOLKIT's Svelte bindings. The AI TOOLKIT's [ KhulnaSoft AI Gateway provider ](/providers/ai-toolkit-providers/ai-gateway) ships with the `ai` package. You'll also install `zod`, a schema validation library used for defining tool inputs.

<Note>
  This guide uses the KhulnaSoft AI Gateway provider so you can access hundreds of
...
Create a `.env.local` file in your project root and add your AI Gateway API key. This key authenticates your application with the KhulnaSoft AI Gateway.
...
Replace `xxxxxxxxx` with your actual KhulnaSoft AI Gateway API key.

<Note className="mb-4">
  The AI TOOLKIT's KhulnaSoft AI Gateway Provider will default to using the
...
This quickstart uses the [KhulnaSoft AI Gateway](https://vercel.com/ai-gateway) provider, which is the default [global provider](/docs/ai-toolkit-core/provider-management#global-provider-configuration). This means you can access models using a simple string in the model configuration:
