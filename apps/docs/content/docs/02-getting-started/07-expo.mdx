...
Install `ai` and `@ai-toolkit/react`, the AI package and AI TOOLKIT's React hooks. The AI TOOLKIT's [ KhulnaSoft AI Gateway provider ](/providers/ai-toolkit-providers/ai-gateway) ships with the `ai` package. You'll also install `zod`, a schema validation library used for defining tool inputs.
