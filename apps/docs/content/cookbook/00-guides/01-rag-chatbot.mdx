...
- [ AI TOOLKIT ](/docs)
- [ KhulnaSoft AI Gateway ](/providers/ai-toolkit-providers/ai-gateway)
...
### KhulnaSoft AI Gateway Key

For this guide, you will need a KhulnaSoft AI Gateway API key, which gives you access to hundreds of models from different providers with one API key. If you haven't obtained your KhulnaSoft AI Gateway API key, you can do so by [signing up](https://vercel.com/d?to=%2F%5Bteam%5D%2F%7E%2Fai&title=Go+to+AI+Gateway) on the Vercel website.

<Note>
  The AI TOOLKIT's KhulnaSoft AI Gateway Provider is the default global provider, so
...
Replace `your-api-key` with your actual KhulnaSoft AI Gateway API key.
...
The `useChat` hook enables the streaming of chat messages from your AI provider (you will be using OpenAI via the KhulnaSoft AI Gateway), manages the state for chat input, and updates the UI automatically as new messages are received.
